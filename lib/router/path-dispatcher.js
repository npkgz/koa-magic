...
function createPathDisptacher(httpMethod, fullMatch=true){
    ...
    return function route(path, middleware){
        ...
        return function dispatchRoute(ctx, next){
            ...
            // current (active path)
            const currentSubPath = ctx._subPath || '';
            const currentPath = currentSubPath || ctx.path || '';
            ...
            ctx._subPath = currentPath.substr(matchedPart.length);
